 ...
 */
async function fetchAsJson(url, fetchOptions, {alternativeFetch = undefined} = {}) {
  return await debugPerformance(url, async parentPerformance => {
    const {timeItAsync} = makeTiming({parentPerformance})
    ...
    const response = await fetchBySubstituteDnsNameWithIp(
      finalFetch,
      url,
      merge({headers: {Accept: 'application/json'}}, fetchOptions),
      parentPerformance,
    )
 ...
 */
async function fetchAsBufferOrJsonWithJsonBody(url, json, fetchOptions, {alternativeFetch} = {}) {
  return await debugPerformance(url, async parentPerformance => {
    const {timeItAsync} = makeTiming({parentPerformance})
    const finalFetch = alternativeFetch || fetch
    const response = await fetchBySubstituteDnsNameWithIp(
    ...
    return await timeItAsync('body', () => (isJson ? response.json() : response.buffer()))
  })
}
...
module.exports = {
  ...
  fetchAsBufferOrJsonWithJsonBody,
  fetch: fetchWithIp,
  retryFetch,
